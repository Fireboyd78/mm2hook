...
protected:
    LPVOID lpAddr;
public:
    inline IHookPtr() : lpAddr(nullptr) {};
    inline IHookPtr(LPVOID lpAddr) : lpAddr(lpAddr) {};
    inline IHookPtr(DWORD dwAddress) : lpAddr((LPVOID)dwAddress) {};
    ...
    inline DWORD operator *() const {
        return *static_cast<DWORD*>(lpAddr);
    };
